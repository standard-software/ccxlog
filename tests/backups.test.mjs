...
test('--backup-jsonl preserves structure and always includes subagents', t => {
  const ws = workspace(t);
  ...
  const r = run([ws.project, '--out', ws.out, '--backup-jsonl'], { home: ws.home });
  assert.equal(r.status, 0, r.stderr);
  assert.match(r.stdout, /Backed up 3 jsonl file/);
  const stamp = onlyStamp(path.join(ws.out, 'backup_jsonl'));
  assert.equal(exists(path.join(stamp, 'cc', 'sess1.jsonl')), true);
  assert.equal(exists(path.join(stamp, 'cc', 'sess1', 'subagents', 'agent-a1.jsonl')), true);
  assert.equal(exists(path.join(stamp, 'cx', '2026', '05', '27', 'rollout-x.jsonl')), true);
});

test('a structure-preserving snapshot restores via extraLogDirs cc/cx after source loss', t => {
  const ws = workspace(t);
  writeJsonl(path.join(ws.ccLogs, 'sess1.jsonl'), claudeQA(ws.project, { q: 'CLAUDE-MAIN-Q' }));
  writeJsonl(path.join(ws.ccLogs, 'sess1', 'subagents', 'agent-a1.jsonl'), sidechainRecords(ws.project));
  writeJsonl(path.join(ws.cxLogs, '2026', '05', '27', 'rollout-x.jsonl'), codexQA(ws.project, { q: 'CODEX-MAIN-Q' }));
  writeConfig(ws.out, { claude: { extraLogDirs: [ws.ccLogs] }, codex: { extraLogDirs: [ws.cxLogs] } });
  ...
  writeConfig(ws.out, {
    claude: { extraLogDirs: [path.join(stamp, 'cc')], includeSidechain: true },
    codex: { extraLogDirs: [path.join(stamp, 'cx')] },
  });
