...
### 1.8.0
#### 2026/08/09(日)
- サブエージェントの会話を描画するかを両ソース共通の設定名で切り替える
  `codex.includeSubagents` / `claude.includeSubagents` を追加。既定はどちらも
  `true` で、Codex の出力は従来と同一、Claude の出力はこれまで省かれていた
  サブエージェントのブロックが加わるだけで、既存の `ccxlogid` は変わらない
- `claude.includeSidechain` は旧名として引き続き使用可能。両方に異なる値を
  指定した場合は、どちらかを黙って優先せず設定エラーとする
- サブエージェントのログは設定に関わらず `--backup-jsonl` で常にバックアップし、
  `false` は継承履歴の統合を終えた後に隠すため、減るのは表示量だけで
  読み込む量は変わらない
- Codex のサブエージェントから親へ返る回答を、Claude の Task 結果と同様に、
  委譲したブロックの Progress として扱うようにした。従来は独立したブロックを
  作るため委譲が2つに分割され、`codex.includeSubagents: false` でも
  サブエージェントの発言が残っていた

### 1.7.1
#### 2026/08/07(金)
- Codex スレッドの表示名に、リネーム後の名前ではなく最初の質問文が使われる
  問題を修正。リネームは `session_index.jsonl` へ先に記録され、ライブ
  データベースの `title` は自動生成タイトルも兼ねて遅れて更新されるため、
  リネームの記録を優先するようにした
